...
class HDEMUCS(pl.LightningModule):
    """
    HDEMUCS: A PyTorch Lightning module implementing HDEMUCS for music source separation.
    ...
    Attributes:
        model (nn.Module): The HDEMUCS model for music source separation.
    ...
    def _modify_grad_transfer_learning(self) -> None:
        ...
        num_params = len([param for param in self.model.parameters()])
        # for transfer learning we will freeze all of the layers except two
        # last ones
        num_params -= 2
        for param, _ in zip(self.model.parameters(), range(num_params)):
