...
def load_audio(file, sr, offset, duration, resample=True, approx=False, time_base="samples", check_duration=True):
    ...
    for frame in container.decode(audio=0):  # Only first audio stream
        ...
        # frame = np.array(frame[0], dtype=np.float16)
        # frame = np.concatenate([f.to_ndarray(format="fltp") for f in frame], axis=1)
        frame = frame[0].to_ndarray()  # Convert to floats and not int16
        read = frame.shape[-1]
